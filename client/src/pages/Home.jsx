    ...
    return (
        <div className='px-2.5 pb-24 '>
            ...
            <div className='max-w-[800px] m-auto rounded-2xl bg-[linear-gradient(rgba(84,3,255,0.15),rgba(105,2,153,0.15))]'>
                <div className="grid grid-cols-[0.5fr_2.5fr_1fr_1fr] sm:grid-cols-[0.5fr_1.5fr_1fr_1fr_1.5fr_1fr] py-4 px-5 items-center border-b border-b-[#3c3c3c]">
                    ...
                    <p className='hidden text-right sm:flex'>Last Update</p>
                </div>
                {displayCoin.map((item, index) => {
                    return <Link to={`/coin/${item.name.toLowerCase()}`} key={index} className='grid grid-cols-[0.5fr_2.5fr_1fr_1fr] sm:grid-cols-[0.5fr_1.5fr_1fr_1fr_1.5fr_1fr] py-4 px-5 items-center border-b border-b-[#3c3c3c] last:border-none'>
                        ...
                        <p className='hidden sm:flex '>{new Date(item.timestamps).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                        })}</p>
